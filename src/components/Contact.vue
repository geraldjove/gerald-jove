<script setup></script>

<template>
  <section class="min-h-[100vh] bg-[#1a1a1a] flex">
    <div
      class="sm:min-h-screen max-w-screen-xl mx-auto text-white flex flex-col items-center justify-center p-4"
    >
      <div><h1 class="font-monoton sm:text-7xl text-5xl">Contact</h1></div>
      <div
        class="bg-[#009efa] w-[120px] sm:w-[300px] sm:h-[10px] h-[5px] mb-5"
      ></div>
      <div class="grid sm:grid-cols-5 p-5 min-h-[500px]">
        <div class="sm:col-span-3 flex flex-col items-center justify-center">
          <h1
            class="sm:text-8xl text-2xl font-black tracking-wider text-white uppercase"
          >
            Let's Bring Your
            <span class="text-[#009efa] italic">Vision</span> into
            <span class="text-[#009efa] italic">Reality</span>!
          </h1>
        </div>
        <div
          class="bg-[#009efa] rounded-xl flex flex-col items-center sm:col-span-2"
        >
          <button
            class="bg-black text-[#009efa] font-bold p-5 rounded-full min-w-[80%] mt-auto mb-10"
          >
            Get in Touch
          </button>
          <h1 class="font-bold text-3xl text-black">OR</h1>
          <button
            class="bg-black p-5 rounded-full min-w-[80%] mb-auto my-10 text-[#009efa] font-bold"
          >
            Email
          </button>
        </div>
      </div>
    </div>
  </section>
</template>
